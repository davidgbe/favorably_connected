...
def treadmill_session_default_params() -> TreadmillEnvParams:
    ...
    return TreadmillEnvParams(
        num_patch_types=3,
        obs_size=5, # 1 visual cue + 3 odor cues + 1 for attempted reward cue
...
def TreadmillEnvironment():
    ...
    def get_observations(key: jnp.ndarray, state: TreadmillEnvState, params: TreadmillEnvParams) -> jnp.ndarray:
        ...
        # Persistent cue: add 1.0 if agent is attempting to collect reward
        obs = obs.at[1 + params.num_patch_types].set(obs[1 + params.num_patch_types] + jnp.where(state.current_reward_site_attempted, 1.0, 0.0))
