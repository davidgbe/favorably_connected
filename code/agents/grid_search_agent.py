...
class GridSearchAgent():
    ...
    def __init__(self, n_envs, wait_time_for_reward, odor_cues_indices, patch_cue_idx, stop_ranges_per_patch, strategy='reward_count'):
        ...
        self.rewards_in_patch = np.zeros((n_envs), dtype=float)
        ...
        self.optimized_n_stops_for_patch = np.zeros((n_patches,), dtype=int)
    ...
    def sample_action(self, observations):
        ...
        if self.search_finished:
            ...
            for k in np.arange(self.n_envs):
                stops_for_patch[k] = self.optimized_n_stops_for_patch[self.current_patch_type[k]]
        else:
    ...
    def cue_session_end(self):
        if not self.search_finished:
            ...
            if (self.n_stops_for_patch == self.stop_ranges_per_patch[:, 1]).all():
                ...
                optimal_policy_idx = self.reward_rates_for_policies.mean(axis=0).argmax()
                self.optimized_n_stops_for_patch = self.n_stops_for_policies[:, optimal_policy_idx]
                print('Optimized stops')
                print(self.optimized_n_stops_for_patch)
                print('Avg reward rate', self.reward_rates_for_policies.mean(axis=0)[optimal_policy_idx])
            else:
                idx = 0
