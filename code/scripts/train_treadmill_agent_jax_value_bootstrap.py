...
# Differential value bootstrap: n-step horizon k for the advantage
#   A_t = sum_{i=0}^{k-1} gamma^i (r_{t+i} - rho_{t+i}) + gamma^k V_{t+k} - V_t,
# with rho = exp_filtered_reward_rate (the average reward E[r]) and V = the critic head.
BOOTSTRAP_K = 100
# Actor single-timestep reward predictor E[r_t | F_{t-1}] (readout on the actor hidden state), weight
# on its MSE loss.
REWARD_PRED_WEIGHT = 1.0
# "Recent luck" baseline: L_t = EMA(r - E[r]) = a filtered reward-prediction error read strictly from
# the past (action-independent -> unbiased). Uses GLOBAL_REWARD_DECAY and persists across update blocks
# (carried in train_state.luck_filter, reset each session). Subtracted from the advantage.
...
def compute_a2c_loss(
    ...
    # --- differential value bootstrap (average-reward n-step advantage) --------------------------
    # rho_t = running average reward rate (exp_filtered_reward_rate) plays the role of E[r_t].
    #   k-step differential return  G_t = sum_{i=0}^{k-1} gamma^i (r_{t+i} - rho_{t+i}) + gamma^k V_{t+k}
    #   advantage                   A_t = G_t - V_t
    # V (the critic head) is fit by MSE to stop_gradient(G_t) (semi-gradient TD). At gamma = 1 this is
    # exactly  sum_{t'=t}^{t+k-1} (r_t' - rho_t') + V_{t+k} - V_t.  Because E[r] = rho is the *average*
    # reward (not a state-conditional one-step predictor), V remains a genuine differential value.
    k = min(BOOTSTRAP_K, N - 1)                                       # bootstrap horizon
    V = trajectory.values                                            # (B, N)  differential value V_t (critic head)
    rho = lax.stop_gradient(trajectory.exp_filtered_reward_rate)      # (B, N)  average reward rate  E[r]
    # --- "recent luck" baseline: L_t = EMA(r - E[r]) read strictly from the past ------------------
    # E[r_t | F_{t-1}] is a single-timestep reward prediction from the ACTOR hidden state (which at t
    # has not observed r_t), trained by MSE below. L is the parallel filtration of that prediction
    # subtracted from the filtered reward -- i.e. a low-passed reward-prediction error: how lucky have
    # I been recently. Because it uses innovations only up to t-1 it does not depend on the current
    # action, so subtracting it from the advantage is an unbiased (variance-reducing) baseline.
    ...
    r_pred = reward_net.apply(params, lax.stop_gradient(trajectory.actor_hidden),
                              method=A2CRNNFlax.predict_reward)[:, :, 0]        # (B, N)  E[r_t | F_{t-1}]
    reward_pred_loss = jnp.mean((r_pred - lax.stop_gradient(rewards)) ** 2)     # train the readout (MSE to r_t)

    innov = lax.stop_gradient(rewards - r_pred)                       # (B, N)  reward-prediction error (luck)
    def _luck_scan(carry, x):                                        # forward EMA over time (same decay as rho)
        L_new = GLOBAL_REWARD_DECAY * carry + (1.0 - GLOBAL_REWARD_DECAY) * x
        return L_new, carry                                          # emit L up to t-1 (strictly past)
    # seed from train_state.luck_filter so L carries across the update blocks within a session
    final_luck, luck = lax.scan(_luck_scan, lax.stop_gradient(train_state.luck_filter),
                                jnp.swapaxes(innov, 0, 1))
    luck = jnp.swapaxes(luck, 0, 1)                                  # (B, N)  L_t = filtered luck before t

    # reward term: sum_{i=0}^{k-1} gamma^i (r_{t+i} - rho_{t+i}), clipped at the chunk end
    i_idx = jnp.arange(k)                                            # (k,)
    s_idx = jnp.clip(jnp.arange(N)[:, None] + i_idx[None, :], 0, N - 1)  # (N, k)
    disc = gamma ** (i_idx)                                   # (k,)
    diff_reward = lax.stop_gradient(jnp.where(rewards > 0, rewards + 0.75 * luck / (1 - GLOBAL_REWARD_DECAY), jnp.zeros_like(rewards)))                    # (B, N)  r_t - rho_t
    reward_term = jnp.sum(diff_reward[:, s_idx] * disc[None, None, :], axis=-1)  # (B, N)



    # bootstrap term: gamma^k V_{t+k}  (stop-grad the bootstrapped value -> semi-gradient TD target)
    tk_idx = jnp.clip(jnp.arange(N) + k, 0, N - 1)                    # (N,)  t -> t+k (clipped)
    V_boot = lax.stop_gradient(V)[:, tk_idx]                          # (B, N)
    G = reward_term + V_boot                                          # (B, N)  k-step differential return / V-target

    critic_loss = jnp.mean((V - lax.stop_gradient(G)) ** 2)                             # fit V_t -> G_t (target stop-gradded above)
    advantages = lax.stop_gradient(G - V - luck)                     # A_t = G_t - V_t - L_t  (recent-luck baseline)
    # advantages = (advantages - advantages.mean(axis=0)) / (advantages.std(axis=0) + 1e-6)  # optional per-t baseline
    ...
    total_loss = (
        ...
        + REWARD_PRED_WEIGHT * reward_pred_loss
    ...
    metrics = {
        ...
        'luck_mean': jnp.mean(luck),
        'luck_std': jnp.std(luck),
    }

    final_train_state = final_train_state.replace(luck_filter=final_luck)   # persist L to the next block
...
class TrainingConfig:
    ...
    reward_pred_lr_scale: float = 1.0  # reward-predictor LR = learning_rate * this; <1 -> slower
    rnn_type: str = 'GRU'
