...
# --- (hidden_state, action) credit filters (persist across update blocks) ---
# Eligibility is written with a bilinear outer product of the actor HIDDEN STATE and the action:
# elig <- low-pass( h_t (x) a_t )  -- a (hidden_size, action_size) matrix. credit is a low-pass of
# (eligibility * signal), where the signal is the TD / reward-prediction error  reward - relu(r_pred).
# The actor is weighted by the readout  h_t . credit[:, a_taken]  -- i.e. the accumulated TD error
# for responding to hidden state h_t with action a_t (see compute_a2c_loss).
...
def compute_a2c_loss(
    ...
    # --- critic: 1-step prediction of the next observation + reward, from the critic hidden state --
    # From critic hidden h^c_t (the pre-step state stored at index t, which has NOT yet consumed o_t),
    # a small MLP head predicts the incoming observation and its reward: [o_t, r_t] (obs_size + 1
    # outputs). NOT stop-gradded, so the critic RNN (rnn_critic) is trained to be 1-step predictive.
    # r_pred (used by the credit signal below) is the predicted-reward channel of this prediction --
    # the same role as the old lag-0 reward, so the credit signal stays a proper RPE.
    ...
    next_pred = reward_net.apply(params, trajectory.critic_hidden,
                                 method=A2CRNNFlax.predict_next)             # (B, N, obs_size + 1)

    # 1-step target: the observation entering step t and its reward, [o_t, r_t].
    obs_feat = trajectory.observations[:, :, :obs_size]                      # (B, N, obs_size)  o_t
    next_target = jnp.concatenate((obs_feat, rewards[:, :, None]), axis=-1)  # (B, N, obs_size + 1)
    critic_loss = jnp.mean((next_pred - lax.stop_gradient(next_target)) ** 2)

    # jax.debug.print('{x}', x=(next_pred - lax.stop_gradient(next_target)).mean(axis=(0, 1)).shape)

    # --- (critic hidden_state, action) credit (persists across blocks via train_state) -----------
    # Eligibility is the bilinear outer product h^c_t (x) a_t (a hidden_size x action_size matrix),
    # low-passed over time. credit = low-pass(eligibility * signal), where the signal is the TD /
    # reward-prediction error  reward - relu(r_pred).  The readout  h^c_t . credit[:, a_t]  is the
    # accumulated TD error for responding to critic hidden state h^c_t with action a_t. r_pred =
    # lag-0 column of the reward-predictor readout of the critic hidden state.
    hidden = lax.stop_gradient(trajectory.critic_hidden)              # (B, N, hidden_size)  critic h^c_t
    r_reward = lax.stop_gradient(rewards)                              # (B, N)  actual reward
    r_pred = lax.stop_gradient(next_pred[:, :, obs_size])              # (B, N)  predicted reward (1-step head)

    def credit_scan(carry, xt):
        elig, credit_rew, credit_prd = carry                           # (B, hidden, A) each
        h_t, act_t, rew_t, prd_t = xt                                  # (B, hidden), (B, A), (B,), (B,)
        h_t_normed = h_t / jnp.sqrt(jnp.sum(h_t**2, axis=-1, keepdims=True) + 1e-6) # change h_t to be the length 
        hid_by_action = h_t_normed[:, :, None] * act_t[:, None, :]   # (B, hidden, A)
        elig = CREDIT_ELIG_DECAY * elig + (1.0 - CREDIT_ELIG_DECAY) * hid_by_action
        signal = rew_t - jnp.maximum(prd_t, 0.0)                       # (B,)  TD / reward-prediction error
        credit_rew = CREDIT_DECAY_REWARD * credit_rew + (1 - CREDIT_DECAY_REWARD) * elig * signal[:, None, None]
        credit = credit_rew                                            # (B, hidden, A)
        credit_taken = jnp.sum(credit * act_t[:, None, :], axis=-1)    # (B, hidden)
        credit_t = jnp.sum(h_t_normed * credit_taken, axis=-1)               # (B,)  h_t . credit[:, a_t]
    ...
    (final_elig, final_credit_rew, final_credit_prd), credit_tN = lax.scan(
        ...
        (jnp.swapaxes(hidden, 0, 1), jnp.swapaxes(act_oh, 0, 1),
    ...
    # advantages = credit_for_action * 100

    # jax.debug.print('{x}', x = (discounted_future_reward).max())
    # jax.debug.print('{x}', x = (credit_for_action).max())
...
def train_a2c_jax(config: TrainingConfig = None, load_path: str = None):
    ...
    for session_num in trange(config.n_sessions, desc='Sessions'):
        ...
        train_state = train_state.replace(
            ...
            prev_obs=obs,
            action_elig=jnp.zeros((config.num_envs, config.hidden_size, config.action_size)),
            action_credit_reward=jnp.zeros((config.num_envs, config.hidden_size, config.action_size)),
            action_credit_pred=jnp.zeros((config.num_envs, config.hidden_size, config.action_size)),
        )
