...
BOOTSTRAP_K = 100
# Weight on the actor's single-timestep reward-prediction MSE (the readout that replaces the old
# 0.25 * rho / (1 - GLOBAL_REWARD_DECAY) term inside diff_reward).
...
def compute_a2c_loss(
    ...
    # Single-timestep reward prediction from the ACTOR hidden state (a readout head, trained by MSE
    # below): r_pred_t ~ r_t.  (The actor hidden at t has not yet observed r_t, so this is a genuine
    # prediction.)  It replaces the old 0.25 * rho / (1 - GLOBAL_REWARD_DECAY) term inside diff_reward.
    ...
    r_pred = reward_net.apply(params, lax.stop_gradient(trajectory.actor_hidden),
                              method=A2CRNNFlax.predict_reward)[:, :, 0]        # (B, N)  predicted r_t
    reward_pred_loss = jnp.mean((r_pred - lax.stop_gradient(rewards)) ** 2)     # train the readout (MSE to r_t)

    diff_reward = lax.stop_gradient(rewards - jnp.minimum(r_pred, 0))  # (B, N)
    ...
    advantages = lax.stop_gradient(G - V)                            # A_t = G_t - V_t  (no grad into the actor)
