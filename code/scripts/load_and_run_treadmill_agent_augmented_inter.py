...
def objective(trial, var_noise, activity_weight):
    ...
    load_path = 'rl_agent_outputs/train_augmented_rnn_pretrained_2025-02-21_00_09_16_185395_var_noise_0.0001_activity_weight_1/rnn_weights/00350.pth'
    ...
    loaded_subnetwork_pcs = compressed_read(os.path.join(DATA_BASE_DIR, 'stored_pcs_and_weights/subnetwork_pcs.pkl').replace('\\','/'))

    print('sub_network_pcs', loaded_subnetwork_pcs.shape)
        ...
        for session_num in trange(N_SESSIONS, desc='Sessions'):
            ...
            for update_num in trange(N_UPDATES_PER_SESSION, desc='Updates in session'):
                ...
                for step in range(N_STEPS_PER_UPDATE):
                    ...
                    hidden_states, critic_hidden_states, subnetwork_hidden_states = agent.get_state()

                    subnetwork_first_pc = loaded_subnetwork_pcs[0, :]
                    subnetwork_hidden_states = subnetwork_hidden_states.cpu().numpy()

                    # dot_prod = np.dot(subnetwork_hidden_states, subnetwork_first_pc)

                    subnetwork_hidden_states = subnetwork_hidden_states - 0.005 * np.outer(np.ones(NUM_ENVS), subnetwork_first_pc)

                    agent.set_state(hidden_states, critic_hidden_states, torch.from_numpy(subnetwork_hidden_states).float().to(DEVICE))
