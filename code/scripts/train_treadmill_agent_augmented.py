...
parser.add_argument('--subnetwork_frozen', metavar='sf', type=int, default=0)
...
def objective(trial, var_noise, activity_weight):
    ...
    load_path = './results/line_attr_supervised/integration_with_resets_train_v1_2025-02-19_16_17_39_594490_var_noise_5e-05_activity_weight_1e-07/rnn_weights/099999.h5'
    subnetwork.load_state_dict(torch.load(load_path, weights_only=True))
    if args.subnetwork_frozen:
        print('subnetwork grad off')
        subnetwork.turn_off_grad()
    ...
    for session_num in trange(N_SESSIONS, desc='Sessions'):
        ...
        for update_num in trange(N_UPDATES_PER_SESSION, desc='Updates in session'):
            ...
            hidden_states, critic_hidden_states, subnetwork_hidden_states = agent.reset_state()
            agent.set_state(hidden_states, critic_hidden_states, subnetwork_hidden_states)
            ...
            # print(subnetwork.rnn.weight_hh.data.cpu().numpy().sum())
