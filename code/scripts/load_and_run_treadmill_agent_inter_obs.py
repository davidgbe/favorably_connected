...
parser.add_argument('--visual_cue_scale', type=float, default=1)
parser.add_argument('--odor_cue_scale', type=float, default=1)
parser.add_argument('--reward_cue_scale', type=float, default=1)
...
def rescale_obs(obs):
    rescale_vec = np.empty((OBS_SIZE))
    rescale_vec[0] = args.visual_cue_scale
    rescale_vec[1:OBS_SIZE] = args.odor_cue_scale
    return rescale_vec.astype(np.uint8) * obs
...
def objective(trial, var_noise, activity_weight):
    ...
    model_path = os.path.join(DATA_BASE_DIR, args.load_path).replace('\\','/')
        ...
        for session_num in trange(N_SESSIONS, desc='Sessions'):
            ...
            for update_num in trange(N_UPDATES_PER_SESSION, desc='Updates in session'):
                ...
                for step in range(N_STEPS_PER_UPDATE):
                    ...
                    obs = rescale_obs(obs)
                    agent.append_reward(reward.astype('float32') * args.reward_cue_scale)
