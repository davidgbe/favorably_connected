...
from train_treadmill_agent_jax import (
    run_session_updates_with_metrics_and_traj,
...
def train_curriculum(config: CurriculumConfig, checkpoint_path: str = None):
    ...
    for step_idx, step in enumerate(config.curriculum):
        ...
        for session_in_step in trange(step.n_sessions, desc=f'Step {step_idx}'):
            ...
            train_state, env_states, all_metrics, training_trajectory = run_session_updates_with_metrics_and_traj(
                ...
                n_save_envs=config.n_save_envs,
            )
            ...
            if should_save(session_in_step, step.n_sessions, step.output_save_start,
                ...
                # training_trajectory has shape (n_save_envs, n_updates*n_steps, ...) per field
                traj_dicts = [
                    serialization.to_state_dict(
                        jax.tree_util.tree_map(lambda x: x[i], training_trajectory)
                    )
                ...
                print(f'  -> Saved training trajectory to {traj_path}')
